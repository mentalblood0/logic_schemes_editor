...
class BlocksArea extends React.Component {
		...
		this.setState(state => {
			...
			for (const b of data.blocks) {
				...
				if (b.type == 'INPUT'){
					...
					const dict_with_blocks_with_such_type = Object.fromEntries(
	...
	updateWireCoordinates(state, wire_id, type_relative_to_block, convert=true) {
		...
		if (type_relative_to_block != 'to')
			wire.from_point =
					convertCoordinates(from_block.output_connectors_coordinates[wire.from_output_id]);
		if (type_relative_to_block != 'from')
			wire.to_point =
					convertCoordinates(to_block.input_connectors_coordinates[wire.to_input_id]);
	...
	getTypeInfo(type_name) {
		if (type_name in default_elements)
			return Object.assign({}, default_elements[type_name]);
		else if (type_name in this.state.custom_elements)
			return  Object.assign({}, this.state.custom_elements[type_name]);
	}
	...
	updateInputsOutputsNames(type, const_id, delta) {
		this.setState(state => {
			...
			if (current_n.includes('-')) {
				...
				if (new_n_to == current_n_from)
					new_current_n = '' + current_n_from;
				else
					new_current_n = current_n_from + '-' + (current_n_to + delta);
			}
			else {
		...
		return <div className="blocksArea" ref={this._ref}>
			<div className="sidePanel"
				...
				<div className="newBlockConfiguration">
					...
					<div className="inputsOutputsGroups">
						<div className="inputsGroups">
						{
							this.state.new_element.inputs_groups.map((g, i) => 
								<div key={i} className="inputGroup unselectable"
									onMouseDown={e => {
										let f = undefined;
										if (e.button == 0)
											f = joinWithNext;
									...
									onContextMenu={e => e.preventDefault()}>{g}</div>
							)
						}
						</div>
						<div className="outputsGroups">
						{
							this.state.new_element.outputs_groups.map((g, i) => 
								<div key={i} className="outputGroup unselectable" 
									onMouseDown={e => {
										let f = undefined;
										if (e.button == 0)
											f = joinWithNext;
					...
					}}>
				{
					Object.entries(this.state.blocks).map(
						...
						<Block key={((block_id_and_block[1].type == 'INPUT') || (block_id_and_block[1].type == 'OUTPUT')) ?
							...
							getTypeInfo={this.getTypeInfo.bind(this)}></Block>
					)
				}
				{
